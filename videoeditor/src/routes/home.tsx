...
import { getAssetIdFromRawUrl } from "~/services/assetsApi";
...
const mediaKey = (value: string | null | undefined) => (value || "").trim().toLowerCase();
...
export default function TimelineEditor() {
  ...
  const { selectedScrubberIds, setSelectedScrubberIds, isChatMinimized, setIsChatMinimized } = useUIStore();
    ...
    (async () => {
      ...
      try {
        if (Array.isArray(j.mediaBinItems) && j.mediaBinItems.length) {
          // Restore ALL media bin items from server
          const restoredItems: typeof mediaBinItems = j.mediaBinItems.map((t: MediaBinItem) => ({
            id: t.id,
            name: t.name,
            ...
            text: t.text || null,
            mediaUrlLocal: null,
            mediaUrlRemote: t.mediaUrlRemote || null,
            durationInSeconds: Number(t.durationInSeconds) || 0,
            isUploading: false,
            uploadProgress: null,
            left_transition_id: t.left_transition_id || null,
            right_transition_id: t.right_transition_id || null,
            groupped_scrubbers: t.groupped_scrubbers || null,
            subtitleData: t.subtitleData || null,
          }));
          upsertMediaItems(restoredItems);
        } else if (Array.isArray(j.textBinItems) && j.textBinItems.length) {
          ...
          if (textItems.length) upsertMediaItems(textItems);
        }
      } catch {
  ...
  useEffect(() => {
    ...
    const hydratedAssets = mediaBinItems.filter(
      (item) => item.mediaType !== "text" && item.mediaType !== "groupped_scrubber" && item.mediaUrlRemote,
    );
    const assetsById = new Map(hydratedAssets.map((item) => [item.id, item]));
    ...
    const newTracks = current.tracks.map((track) => ({
      ...
      scrubbers: track.scrubbers.map((s) => {
        ...
        const assetIdFromRemote = getAssetIdFromRawUrl(s.mediaUrlRemote);
        const match =
          assetsById.get(s.sourceMediaBinId) ||
          (assetIdFromRemote ? assetsById.get(assetIdFromRemote) : undefined) ||
          (s.mediaUrlRemote ? assetsByRemote.get(s.mediaUrlRemote) : undefined) ||
          assetsByName.get(mediaKey(s.name));

        if (!match || !match.mediaUrlRemote) {
          return s;
        }

        const nextScrubber: ScrubberState = {
          ...s,
          mediaUrlLocal: null,
          ...
          durationInSeconds: match.durationInSeconds || s.durationInSeconds,
        };

        if (
          nextScrubber.mediaUrlLocal !== s.mediaUrlLocal ||
          nextScrubber.mediaUrlRemote !== s.mediaUrlRemote ||
          nextScrubber.sourceMediaBinId !== s.sourceMediaBinId ||
          nextScrubber.media_width !== s.media_width ||
          nextScrubber.media_height !== s.media_height ||
          nextScrubber.durationInSeconds !== s.durationInSeconds
        ) {
          changed = true;
          return nextScrubber;
        }
  ...
  const handleSplitClick = useTimelineSplit({
    ...
    getPixelsPerSecond,
    getAllScrubbers,
    splitScrubberAtRuler: handleSplitScrubberAtRuler,
    addMediaItems,
  });
                  ...
                  <div className="h-8 border-b border-border/50 bg-muted/30 flex items-center justify-between px-3 shrink-0">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        data-preserve-scrubber-selection="true"
