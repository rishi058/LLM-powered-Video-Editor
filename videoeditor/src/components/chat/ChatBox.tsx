...
export function ChatBox({
  ...
  const loadTabs = (): ChatTab[] => {
      ...
      if (Array.isArray(parsed)) {
        return parsed.map((t: Record<string, unknown>) => ({
          ...
          messages: Array.isArray(t.messages)
            ? t.messages.map((m: Record<string, unknown>) => ({
    ...
    } catch (_unused) { /* ignore parse error */ }
    return [];
  };
  ...
  }, [timelineState, handleUpdateScrubber, pendingResizeRequests]);
  ...
  const persistTabs = (next: ChatTab[]) => {
    ...
    } catch (_unused) { /* ignore persist error */ }
  };
  ...
  const handleSendMessage = async (includeAllMedia = false) => {
    ...
    try {
      ...
      let functionCallResponse: { function_call?: { function_name: string; arguments: Record<string, unknown> }; assistant_message?: string };
      try {
        ...
        if (response && typeof response.data === "object" && response.data !== null) {
          ...
          if (data.function_call || data.assistant_message) {
            ...
            functionCallResponse = { assistant_message: "I received an invalid response format from AI." };
          }
        } else {
          functionCallResponse = { assistant_message: "I received an invalid response format from AI." };
        }
      ...
      if (functionCallResponse.function_call) {
        ...
        const toSeconds = (val: unknown): number | undefined => {
          ...
          } catch (_unused) { /* ignore duration parse error */ }
          // Try hh:mm:ss / mm:ss
        ...
        try {
          if (fn === "LLMAddScrubberToTimeline") {
            ...
            const { scrubber_id, ...style } = parsed.data as Record<string, unknown>;
            ...
            const count = toNumber((args as Record<string, unknown>).count) ?? 1;
            if (handleAddTrack) {
            ...
            const startSec = toSeconds((args as Record<string, unknown>).start_seconds) ?? 0;
            const pps = toNumber((args as Record<string, unknown>).pixels_per_second) ?? pixelsPerSecond;
  ...
  const formatTime = (dateLike: unknown) => {
    try {
      const d = dateLike instanceof Date ? dateLike : new Date(dateLike as string | number);
            ...
            <div className="max-h-72 overflow-y-auto p-1">
              {(() => {
                const filtered = tabs
                  .map((t) => ({
                    ...t,
                    lastActivity: (t.messages?.[t.messages.length - 1]?.timestamp as Date | string | number | undefined)?.getTime?.() || t.createdAt,
                  }))
                  .filter((t) => t.name.toLowerCase().includes(historyQuery.toLowerCase()))
                  .sort((a, b) => b.lastActivity - a.lastActivity);

                const groups: Record<string, typeof filtered> = {} as Record<string, typeof filtered>;
                filtered.forEach((t) => {
