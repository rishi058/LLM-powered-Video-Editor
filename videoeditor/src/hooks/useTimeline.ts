...
export const useTimeline = () => {
  const {
    ...
    snapshotTimeline: snapshotTimelineInStore,
    undo,
    redo,
    splitScrubberAtRuler: splitScrubberAtRulerInStore,
  } = useTimelineStore();

  const zoomLevelRef = useRef(zoomLevel);
  // Keep the ref in sync with the zustand store state since it's used in callbacks
  useEffect(() => {
  ...
  const handleUpdateScrubber = useCallback(
    ...
    [deepClone, setTimeline, setUndoStack, setRedoStack],
  );
  ...
  const handleSplitScrubberAtRuler = useCallback(
    (
      ...
      physicalSplitData?: {
        part1: { id: string; mediaUrlRemote: string; durationInSeconds?: number | null; name?: string; width?: number | null; height?: number | null };
        part2: { id: string; mediaUrlRemote: string; durationInSeconds?: number | null; name?: string; width?: number | null; height?: number | null };
      }
    ) => {
      return splitScrubberAtRulerInStore(rulerPositionPx, selectedScrubberId, physicalSplitData);
    },
    [splitScrubberAtRulerInStore],
  );
  ...
  const handleAddTransitionToTrack = useCallback(
    ...
    [getPixelsPerSecond, timeline.tracks, validateTransitionPlacement, snapshotTimeline, setTimeline],
  );
  ...
  const handleDeleteTransition = useCallback(
    ...
    [getPixelsPerSecond, snapshotTimeline, setTimeline],
  );
  ...
  const handleMoveGroupToMediaBin = useCallback(
    ...
    [getAllScrubbers, getPixelsPerSecond, setTimeline],
  );
